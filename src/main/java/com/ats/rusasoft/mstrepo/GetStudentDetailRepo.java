...
public interface GetStudentDetailRepo extends JpaRepository<GetStudentDetail, Integer> {
	...
	@Query(value="SELECT m_student_reg.institute_id ,m_student_reg.student_id,m_student_reg.dept_id,m_student_reg.student_name, m_student_reg.id_no,m_student_reg.contact_no,m_student_reg.email,m_institute.institute_name,m_dept.dept_name ,  m_student_reg.acadamic_year as academic_year  from m_student_reg,m_institute,m_dept where m_student_reg.del_status=1 AND  m_student_reg.institute_id=m_institute.institute_id AND m_dept.dept_id=m_student_reg.dept_id and   m_student_reg.institute_id =:instituteId ORDER BY m_student_reg.student_id DESC",nativeQuery=true)

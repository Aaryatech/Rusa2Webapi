...
public interface GetInstituteListRepo extends JpaRepository<GetInstituteList, Integer> {
	@Query(value = " SELECT m_institute.institute_id, m_institute.institute_name,m_institute.principal_name,m_institute.email,m_institute.contact_no from m_institute,t_user_login WHERE m_institute.is_active=1 "
			+ " AND m_institute.del_status=1 AND t_user_login.ex_int2=m_institute.institute_id AND t_user_login.user_type=1 ORDER BY institute_id DESC ", nativeQuery = true)

...
public interface GetFacultyPhdGuideRepo extends JpaRepository<GetFacultyPhdGuide, Integer> {
	...
	@Query(value = " SELECT j.*,f.faculty_first_name,d.dept_name FROM t_faculty_phdguide j,m_faculty f,m_dept d "
			...
			+ "f.dept_id=d.dept_id AND f.institute_id=:instituteId   ORDER BY j.phd_id DESC ", nativeQuery = true)
	...
	@Query(value = "  SELECT j.*,f.faculty_first_name,d.dept_name FROM t_faculty_phdguide j,m_faculty f,m_dept d WHERE j.del_status=1  "
			+ "	  AND j.year_id=:yearId AND j.is_active=1 AND f.faculty_id=j.faculty_id AND "
			+ "f.dept_id=d.dept_id AND d.dept_id IN(:deptIdList) AND f.institute_id=:instituteId  ORDER BY j.phd_id DESC ", nativeQuery = true)
	...
	@Query(value = "  SELECT j.*,f.faculty_first_name,d.dept_name FROM t_faculty_phdguide j,m_faculty f,m_dept d WHERE j.del_status=1  "
			...
			+ "AND f.institute_id=:instituteId  ORDER BY j.phd_id DESC ", nativeQuery = true)

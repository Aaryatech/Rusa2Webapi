...
public interface GetAlumniRepo  extends JpaRepository<GetAlumni, Integer>{
	...
	@Query(value=" SELECT t_alumni_detail.alumni_detail_id,t_alumni_detail.institute_id,t_alumni_detail.alumni_name, "
			...
			+ " WHERE t_alumni_detail.institute_id=:instId AND t_alumni_detail.year_id=:yearId AND t_alumni_detail.del_status=1 AND t_alumni_detail.is_active=1  ", nativeQuery=true)

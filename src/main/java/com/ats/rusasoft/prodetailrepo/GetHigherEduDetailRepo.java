...
public interface GetHigherEduDetailRepo extends JpaRepository<GetHigherEduDetail, Integer> {
	...
	@Query(value = " SELECT t_higher_education_detail.education_detail_id, t_higher_education_detail.no_student, "
			...
			+ " FROM t_higher_education_detail WHERE t_higher_education_detail.year_id=:yearId AND t_higher_education_detail.institute_id=:instId AND t_higher_education_detail.del_status=1 AND t_higher_education_detail.is_active=1 ", nativeQuery = true)

...
public interface FinancialSuppToProfMemRepo extends JpaRepository<FinancialSuppToProfMem , Integer> {
	...
	@Query(value="SELECT\n" + 
			...
			"    m_academic_year\n" + 
			"WHERE\n" + 
			"    faculty_empowerment.inst_id = m_institute.institute_id AND faculty_empowerment.ac_year_id = m_academic_year.year_id AND faculty_empowerment.ex_int1 = m_faculty.faculty_id AND m_faculty.dept_id = m_dept.dept_id AND faculty_empowerment.inst_id =:instId AND faculty_empowerment.ac_year_id IN(:acYearList) AND faculty_empowerment.financial_support = 1  \n" + 
			"ORDER BY `faculty_empowerment`.`faculty_empwrmnt_id` ASC",nativeQuery=true)
	List<FinancialSuppToProfMem> getAllFinancialSuppToProfMem(@Param("instId") int instId,@Param("acYearList") List<Integer> acYearList);
}
